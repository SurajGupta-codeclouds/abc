...
const Chat = ()=>{
    ...
    const searchChat= async ()=>{
        ...
        const data = await axios.get(`http://localhost:1000/api/user?search=${searchData}`, config)
    ...
    const getOldMessage = async ()=>{
		...
		try{
            ...
            const {data} = await axios.get(`http://localhost:1000/api/message/${selectChat._id}`, config);
			setMessages(data)
    ...
    const handleSubmit= async (values)=>{
		...
		try{
            ...
            const {data} = await axios.post("http://localhost:1000/api/message", values, config);
    ...
    return(
        <>
            <Grid container className="containers">
                <Grid item xs={4} md={4} style={{borderRight:'0.1px solid #80808024', padding:'15px'}}>
                ...
                <div class="align-box-row">
                    ...
                    <div style={{marginLeft:'10px'}}>
                        <span class="d-block text-capitalize">
                            {loginUser?.name}
                            <small class="d-block text-black-50 text-initial ">({loginUser?.email})</small>
                        </span>
                    </div>
                ...
                <div className="searchbox">
                    <input type="text" aria-label="search" className="searchInput" placeholder="🔍 name, email" value={searchData} onChange={(e)=> setSearchData(e.target.value)} />
                </div>
                    <MenuList>
                        {
                            searchData?.length === 0 ?
                                            ...
                                            listingData?.map((item)=>(
                                                ...
                                                onClick={()=> setSelectChat(item)}
                                                    style={{
                                                            ...
                                                            borderRadius:'6px',
                                                            background: selectChat === item ? "#dbf1ff":"transparent",
                                                         }}>
                                                    <div class="align-box-row">
                                                        <div class="avatar-icon-wrapper avatar-icon-md">
                                                            <span class="badge badge-circle badge-danger"></span>
                                                            <div class="avatar-icon rounded-circle">
                                                            {`${item?.isGroupChat ? item?.chatName : getSenderName(loginUser, item?.users)}`.match(/\b([A-Za-z])/g)?.join('')}
                                                            </div>
                                                        </div>
                                                        <div style={{marginLeft:'10px'}}>
                                                            <span class="d-block text-capitalize">
                                                                {
                                                                    item?.isGroupChat ? item?.chatName : getSenderName(loginUser, item?.users)
                                                                }
                            ...
                            <>
                                ...
                                </div>
                                {
                                    ...
                                    searchDataResult?.map((item)=>(
                                        <MenuItem style={{borderBottom:'0.1px solid #80808024'}} onClick={()=>handleClick(item?._id)}>
                                            <div class="align-box-row">
                                                <div class="avatar-icon-wrapper avatar-icon-md">
                                                    <span class="badge badge-circle badge-danger"></span>
                                                    <div class="avatar-icon rounded-circle">
                                                ...
                                                <div style={{marginLeft:'10px'}}>
                                                    <span class="d-block text-capitalize">
                                                        {item?.name}
                                                        <small class="d-block text-black-50 text-initial">({item?.email})</small>
                                                    </span>
                                                </div>
                ...
                <Grid item xs={8} md={8}>
                   <RightSideChat isTyping = {isTyping} messages={messages} handleSubmit={(values)=> handleSubmit(values)} typings={()=> typings()} />
                </Grid>
            </Grid>
            <style>{`
